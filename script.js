/* ============================================================
   ...
   ✅ AUCUN PROXY  ✅ AUCUNE CLÉ  ✅ FONCTIONNE SUR ANDROID
   Grâce à WebViewAssetLoader dans MainActivity.java,
   l'app tourne sur une vraie origine HTTPS →
   fetch() vers api.brawlapi.com marche directement.
   ============================================================ */

'use strict';

/* ──────────────────────────────────────────────
   API (directe, sans proxy)
────────────────────────────────────────────── */
const BRAWLAPI = 'https://api.brawlapi.com/v1/players/';
...
/* ──────────────────────────────────────────────
   FETCH API BRAWLAPI — APPEL DIRECT
   Fonctionne grâce à WebViewAssetLoader qui donne
   une origin HTTPS à l'app → pas de blocage CORS
────────────────────────────────────────────── */
async function fetchPlayer(rawTag) {
  ...
  // On essaie les deux formats (sans # puis avec %23)
  const urls = [
    `${BRAWLAPI}${tag}`,
    `${BRAWLAPI}${encodeURIComponent('#' + tag)}`,
  ];
  ...
  for (const url of urls) {
    try {
      ...
      lastErr = new Error(`Erreur API ${res.status}`);
    } catch (e) {
      if (e.message.includes('introuvable')) throw e; // 404 : inutile de réessayer
      lastErr = e;
    }
  }
  throw lastErr || new Error('Impossible de joindre l\'API Brawl Stars.');
}
